...
<body>
	...
	<script>
	    ...
	    var btn = document.getElementById("chat-ai-btn");
	    ...
	    // Send 버튼 클릭 시 ChatGPT와 대화
	    $('#sendChatBtn').click(function() {
	        var userMessage = $('#userInput').val().trim();
	
	        if (userMessage) {
	            // 사용자가 입력한 메시지를 채팅 화면에 추가
	            ...
	            $.ajax({
	                ...
	                success: function(response) {
	                    // ChatGPT의 응답을 채팅 화면에 추가
	                    $('#chatContent').append('<p><strong>AI:</strong> ' + response + '</p>');
	                    $('#chatContent').scrollTop($('#chatContent')[0].scrollHeight); // 스크롤 하단으로 이동
	                },
	    ...
	    // Enter 키를 눌렀을 때도 메시지를 전송하도록 설정
	    $('#userInput').keypress(function(e) {
	        if (e.which == 13) { // Enter 키의 keycode
	            $('#sendChatBtn').click();
	        }
	    });
	});
		...
		function saveData() {
			...
			$('#drugPrescriptions tbody tr').each(
				function() {
					...
					const ingdNameKor = $(this).find('td:nth-child(2)').text();
					const fomlNm = $(this).find('td:nth-child(3)').text();
					const dosageRouteCode = $(this).find('td:nth-child(4)').text();
					const dayMaxDosgQyUnit = $(this).find('td:nth-child(5)').text();
					const dayMaxDosgQy = $(this).find('td:nth-child(6)').text();
					drugData.push({
						cpntCd: cpntCd,
